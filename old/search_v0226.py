...
with pd.ExcelWriter(output_excel) as writer:
    for tab in tabs:
        ...
        for i in tqdm.tqdm(range(len(df)),desc=tab):
            ...
            documents = vectorstore.similarity_search_with_relevance_scores(project_name, k=50)
            documents_pass = [(doc,score) for doc,score in documents if doc.metadata['if_pass'] == 'true']
    
            for j, (doc, score) in enumerate(documents_pass):
                if j >= RECOMMAND_AMOUNT:
                    break
